...
fields = ["Trees.ID", "Trees.maori_name", "Trees.reference", "Trees.scientific_name", "Employees.ID", "Employees.first_name", "Employees.last_name", "Employees.email", "Employees.gender", "Employees.employerID"]
...
def Main():
	while True:
		...
		if action == "1":
			while True:
				querytable = AskWhichTableToAddIn()
				try: AskForEachFieldForAdding_and_ExecuteAddingQueries(querytable)	
				except: break
		...
		if action =="2":
			while True:
				...
				shownfields_nameslist = AskForDisplayedFieldsForSearching()
				try: filters = AskFilters(shownfields_nameslist)
				except: break
				try: ExecuteSearchingQuery(filters)
				except: pass

def AskWhichTableToAddIn():
	update()
	while True:
...
def AskForEachFieldForAdding_and_ExecuteAddingQueries(querytable):
	...
	if querytable == "Employees":
		...
		c.execute("""SELECT Trees.Name, Employees.ID, Employees.first_name, Employees.last_name FROM Trees
					INNER JOIN Employees
					ON Trees.ID = Employees.employerID
					WHERE Trees.ID = ?;""", (addinglist[4],))
		headers = ["Trees.Name", "Employees.ID", "Employees.first_name", "Employees.last_name"]
...
def AskForDisplayedFieldsForSearching():
	global queryFields
	...
	while True:
		...
		queryFields = ""
		for i in list: 
			try: queryFields += fields[int(i)-1] + ", "
			except: valid = False
		if valid: break
...
def AskFilters(shownfields_nameslist):
	...
	for field in shownfields_nameslist:
		while True:
			...
			if temp == backkey:
				return
				AskForDisplayedFieldsForSearching()
			if temp == "": break
			try: 
				temp = int(temp)
				break
			except: 
				if 'ID' in field: pass # stay in while loop and re-input bc IDs cant be letters
				else: 
